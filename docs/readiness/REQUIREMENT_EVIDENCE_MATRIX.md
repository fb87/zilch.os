...
| SCH-009/010/016 | COMPLETE | `src/kernel/include/sys/kernel/scheduling/context.hh`, `docs/readiness/SCHEDULING_CONTEXT.md` | real-time-stamped per-slice and donated-budget replenishment with progress-safe queue overflow, overflow-safe configuration, and lock-order hold-time telemetry | `sporadic_server_replenishment`, donation/timeout/pager suites, `lock_hold_measurement`, and full root-only acceptance pass | IRQ-disabled timing remains SCH-017 |
