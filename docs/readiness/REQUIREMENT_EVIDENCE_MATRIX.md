...
| CAP-003..005 | COMPLETE | `docs/readiness/CSPACE_DESIGN.md` | two-level radix, eight-bit guards, per-leaf occupancy bitmaps, rotating allocation hint in `capability/cspace.hh` | `guarded_cspace_scale` spans 193 slots and all four leaves; wrong-guard negative passes | CSpace capacity is intentionally bounded at 256 slots; derivation storage scalability is tracked separately by CAP-008/013 |
| CAP-020 | COMPLETE | `docs/readiness/CSPACE_DESIGN.md` | guarded cross-CSpace copy/lookup/delete and bitmap reuse | `cross_cspace_transfer_fuzz` passes 4,096 generated operations; existing `capability_transfer_revoke_race` remains green | Multi-capability atomic IPC transfer remains CAP-017..019 work |
| CAP-016 | COMPLETE | generation-tagged object and derivation lifetime | derivation handles encode a non-wrapping generation; records with live children cannot be reused; inactive intermediate records retain exact-generation ancestry | `derivation_generation_aba`, `object_lookup_destroy_race`, `capability_transfer_revoke_race`, and four-CPU destroy/reuse acceptance PASS | Storage remains bounded under CAP-008/013; exhausted generations fail closed |
...
| SCH-017/020..023 | IN PROGRESS | `src/kernel/include/sys/kernel/interrupt/timing.hh`, `src/arch/arm64/arch.cc`, `syscall/ipc.hh` | per-CPU IRQ-masked, IRQ-service, preemption-service, cross-CPU-wake, and IPC-service maxima and sample counts | full certification emits nonzero `[METRIC]` records for every class without changing functional acceptance | QEMU host scheduling produces nondeterministic elapsed-time outliers; stable targets require retained real-hardware runs |
...
| IPC-005/006 | IN PROGRESS | endpoint-to-lifecycle lock protocol | cancellation locks the resolved endpoint, locks IPC lifecycle, revalidates state and selector, removes membership, clears reply/donation state, and wakes before releasing in reverse order | `ipc_lifecycle_races`, blocked destroy/reuse, root SMP fuzz, and full four-CPU acceptance PASS | Controlled instruction-level cancellation versus dequeue/requeue stress remains |
| CAP-015 / MEM-016 | IN PROGRESS | authority retirement followed by object grace period | frame/resource/page-table destruction excludes map/revoke/release, frame retirement is published under the mapping lock, and authority is released before reader synchronization | `mapping_database_invariants` at bootstrap and `kernel_lifetime_invariants` after all four-CPU workloads; mapping, memory-authority revoke, destroy/reuse, pager, and pressure suites PASS | Per-object scalable reclamation, scalable mapping index, and forced instruction-level map-versus-revoke scheduling remain |
| CAP-012, CAP-014..015 / MEM-019 | IN PROGRESS | complete process-bundle retirement transaction | target quiescence and resource retirement precede an authority transaction that revalidates control, unmaps the address space, drains the task CSpace, and revokes thread/task/address-space/scheduling-context references before reader grace periods | `object_destroy_reuse`, four-CPU certification, `kernel_lifetime_invariants`, and `process_lifecycle_invariants` PASS | Scalable authority/mapping indexes and long-duration controlled teardown interleavings remain |
| IPC-006/008 / IRQ-002/004 | IN PROGRESS | retire/revoke/quiesce/reuse lifecycle | endpoints publish retirement under endpoint-to-authority locks; stale call/receive fails closed; notifications revoke before grace-period reuse; IRQ registry uses atomic publication and mask-before-rebind | dynamic IPC objects, IPC lifecycle races, object destroy/reuse, IRQ delegation/ack/storm suites, `kernel_lifetime_invariants`, and `interrupt_lifetime_invariants` PASS | Notification waiter policy, controlled stale-endpoint instruction interleaving, and data-driven external IRQ discovery remain |
| MEM-025/026 | IN PROGRESS | `docs/readiness/PAGER_FAILURE_PROTOCOL.md` | lifecycle-serialized idempotent mapping and bounded orphan termination | `same_page_fault_serialization` verifies one mapping record; full certification remains green | forced simultaneous fault race and userspace supervisor policy remain |
...
| IPC-003..016 | COMPLETE | bounded endpoint lifecycle, four-capability authority transaction, frame-grant OOL transport, nonblocking notifications, targeted wakeups, and timing/footprint gates | transfer/revoke forces partial batch rollback; lifecycle races cover cancel/timeout/exit/teardown; final acceptance checks endpoint invariants and IPC latency; release gate counts call/receive/reply instructions | ARM64 four-CPU certification and ARM64/AMD64 production builds PASS | OOL payloads intentionally use memory capabilities rather than kernel buffering; AMD64 SMP wake delivery remains outside the current inactive platform |
...
| SCH-008/009 | IN PROGRESS | `scheduling/context.hh`, `syscall/control.hh`, `scheduling/context.md` | `scheduling_configuration` rejects live mutation, priority truncation, zero budget, and budget-over-period; valid suspended configuration passes | Sporadic-server conformance and long-duration RT stress remain open |
