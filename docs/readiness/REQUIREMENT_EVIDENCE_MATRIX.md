...
| SCH-009/016 | IN PROGRESS | `src/kernel/include/sys/kernel/scheduling/context.hh`, `docs/readiness/SCHEDULING_CONTEXT.md` | bounded periodic replenishment, overflow-safe configuration, and lock-order hold-time telemetry | donation/timeout suites, `lock_hold_measurement`, and full certification acceptance | True sporadic-server replenishment and IRQ-disabled timing remain open |
