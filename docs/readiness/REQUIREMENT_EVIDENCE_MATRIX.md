...
| SCH-009/010/016 | COMPLETE | `src/kernel/include/sys/kernel/scheduling/context.hh`, `docs/readiness/SCHEDULING_CONTEXT.md` | bounded per-slice replenishment queue, overflow-safe configuration, and lock-order hold-time telemetry | `sporadic_server_replenishment`, `lock_hold_measurement`, donation/timeout suites, and full certification acceptance | IRQ-disabled-section coverage remains SCH-017 |
