...
- [x] **PRD-020** Checked-in `configs/{debug,release,guest}_defconfig` exist and select the profile through `KCONFIG_DEFCONFIG`; release makes every test/debug option unavailable (`CONFIG_TESTS`/`VERBOSE_DIAGNOSTICS`/`TRACE`/`DEBUG_INFO` all `depends on BUILD_DEBUG`). The defconfig is now the sole authority for which profile is built, and `BUILD_VARIANT` names an output tree only — `guest_defconfig` is a release build even though smoke places it in the `development` tree, which is precisely why the variant must carry no profile meaning. `BUILD_PROFILE` survives only to pick a default defconfig when none is given, so `make debug` still means `configs/debug_defconfig`. The duplication that actually bit was silent: `mk/config.mk`'s rule depended on the defconfig as a FILE rather than on which one was selected, so a tree kept whatever profile it was last built with. Demonstrated — building a tree with `release_defconfig` then rebuilding with `debug_defconfig` left `CONFIG_TESTS` unset — and fixed with a stamp recording the selection, after which the switch is honoured. `smoke.sh`'s two hand-written cleanups of the generated config are deleted and it still passes.
...
- [x] **USR-036** Per-role restart limits fail closed, zero-limit services cannot restart, unexpected crash accounting is real, and the budget now decays. `restart_count` was monotonic, so a role crashing once an hour eventually exhausted its limit and stayed down for good, indistinguishable from one crash-looping in milliseconds — the limit is meant to stop a flapping service, not cap a long-lived one's lifetime failures. Sustained readiness now refunds one attempt per `restart_decay_iterations` (8192) consecutive supervision-loop iterations; any fault, clean exit or restart resets the counter, so a crash-looper can never reach the threshold and stays bounded exactly as before. Iterations rather than seconds because userspace has **no clock at all** — the loop was measured at ~700 iterations/second on the reference profile, making 8192 roughly ten seconds there, and the comment says to treat it as an order of magnitude since the rate varies with host speed under TCG. Known limitation: in the guest-embedded profile root's thread commits to `run_embedded_guest_loop()` after readiness, so the supervision loop — and therefore the decay — does not run in that configuration.
- [x] **USR-037** Every core role publishes a collision-free readiness badge and answers a role-bound health/description RPC, and asynchronous fault records now survive into release. The badge and RPC halves were already done; the fault records were not, for a reason that reads as done in the source: the user-fault path called `emergency::trace()`, which is `append()` under `CONFIG_TRACE` and nothing at all otherwise. Certification builds `debug_defconfig` with `CONFIG_TRACE=y`, so every test and every manual run recorded faults exactly as intended — and `release_defconfig` does not set it, so the one configuration that ships compiled the record away. A fault is precisely what a post-mortem needs, and it is the one event that is never reproducible on request, so the call site now uses `emergency::append()` directly and is no longer conditional. The cost is bounded by construction: the ring is four records per CPU, so a fault storm evicts its own older entries and touches nothing else. Verified by comparing release ELFs rather than by reading the diff — tests are excluded from `release_defconfig`, so the scheduler change is the only thing that can move that binary, and it does (`58206e51…` → `c8c0f717…`); an unchanged hash would have meant the record was still being compiled out. `device_assignment_audit` additionally counts `user_fault` records and fails any carrying `argument[1] == 0`, because a fault record without its ESR cannot distinguish an undefined instruction from a translation fault — which was the difference between two separate defects while chasing the boot stall (0141). That check reports `faults=0` in certification and is expected to: the bootstrap self-test runs before any userspace fault can occur, so it guards the format of records rather than proving the append fires.
...
- [x] **DEV-004** Device reset requirement documented per device, and now enforced by the kernel on revocation. It had been documented and not implemented, and the gap was not cosmetic: unmapping a device frame stops the old owner from *naming* the device, not from running. A virtio transport left with `DRIVER_OK` and a programmed virtqueue still holds the DMA addresses its driver gave it — and `reclaim_task_memory()` has just returned those pages to the free pool, so one more completed request writes into whatever is allocated there next. Each incoming driver does reset its own device during bring-up, which is why this was never observed, but that is the new owner protecting itself rather than the system guaranteeing a clean device, and it does nothing about the window in between or a device left live with no owner at all. The mechanism keeps device knowledge out of the kernel: whoever creates a device frame *declares* a quiesce — an offset, a value, and a repeat (count and stride) — and the kernel guarantees those writes happen on revocation, after the unmap so the old owner cannot re-arm the device in between, and crucially even when the owning driver died and cannot do it itself. Root declares PL011 IMSC←0 (mask every UART interrupt; count 1) and virtio `Status`←0 at 0x070 across all eight transports of the granted page at 0x200 stride. All eight because root cannot know which transport is populated — that is what the driver probes for and reports back, long after the frame was created — and the whole page belongs to that one driver anyway. The declaration is bounds-checked at creation, not at revocation: unchecked it would be an arbitrary-kernel-write primitive for anyone permitted to create device frames, and validating late would be useless precisely when the driver is already gone. This also required mapping the virtio-mmio window into the kernel's tables (it previously mapped only the GIC and PL011), EL1-only and PXN|UXN like its neighbours. Verified by `[TEST] name=device_revocation_quiesce result=PASS transports=8 stride=0x200 neighbours_untouched=1 rejected=6`, which drives the write loop against a poisoned scratch RAM page — pointing it at real MMIO would mean masking the live console or resetting the block device in use to prove an address calculation — and confirms the words outside the declared targets are untouched and that six malformed declarations (misaligned offset, misaligned stride, zero count, start past the page, last write past the page, count×stride overflow) are all rejected. smoke PASS drives the real `serial-restart` and `block-restart` paths, so the writes also fire against real hardware without breaking the restart flow. Remaining: DEV-017 still asks for positive proof of no state leakage between owners; this closes the mechanism half of that. Two devices are assignable today. PL011 UART (`serial-driver`): reset requirement is to mask RX at IMSC, drain the RX FIFO, and clear pending interrupts at ICR before a new owner binds; the kernel now performs the interrupt half on revocation (`interrupt::release_ownership()` masks the line, drops any in-flight delivery and unbinds), and the FIFO half is done by the incoming owner's `configure_uart()`/`drain_rx()` rather than by the outgoing one. virtio-mmio block (`virtio-driver`): reset requirement is a write of 0 to `Status`, which per the virtio spec resets the device and abandons the virtqueue, before a new owner re-negotiates; this is NOT performed on revocation — a restarted driver currently re-initialises the transport from whatever state the previous owner left, which works because it rewrites `Status` during its own bring-up, but is not the same as the host guaranteeing a clean device. Neither device is reset by the kernel on reassignment, so state leakage between owners is prevented only by each incoming driver re-initialising its device. That is what DEV-017 asks to be proven and why it stays open.
...
- [x] **DEV-016** Device revoke stops DMA before teardown completes. The DMA-stopping action is the declared quiesce built for DEV-004 — for virtio, `Status`←0, which per the spec resets the device and abandons its virtqueue — and it runs inside `release_frame_mappings()`, after the unmap and before that function returns, so by the time revocation completes the device is no longer a bus master. The teardown ordering was traced rather than assumed: `destroy_user_bundle()` calls `capability::revoke_all_locked()` (scheduler.hh:1383), which sweeps each slot through `revoke_descendants_locked()`, which invokes the revoked-object hook once every cspace lock is dropped — that hook is `on_capability_revoked()`, and for a device frame it calls `release_frame_mappings()`. `reclaim_task_memory()`, which returns the driver's DMA pages to the free pool, runs at scheduler.hh:1400, strictly after. That is the ordering that matters: the device is reset before the pages its virtqueue points at become reusable. Verified by `[TEST] name=device_revocation_quiesce result=PASS`, which observes the writes have landed by the time `release_frame_mappings()` returns, and by smoke's `block-restart` and `serial-restart` gates driving the real path. Note this fires even though root retains the original frame capability — the driver's minted copy being revoked is what quiesces the device, which is exactly right for restart: device stopped, root's authority intact to re-mint to the fresh instance.
- [x] **DEV-017** Device reset prevents state leakage to the next owner. Closed by the same mechanism (DEV-004): the frame creator declares a reset write and the kernel guarantees it on revocation, so the next owner is handed a device in a known state rather than whatever the previous owner left. Previously this held only because each incoming driver happened to reset its own device during bring-up — the new owner protecting itself, which does nothing for a device left live with no owner at all. The test seeds every transport with a genuine fully negotiated status (`ACKNOWLEDGE|DRIVER|FEATURES_OK|DRIVER_OK`, the state a driver that died mid-service actually leaves behind) rather than an arbitrary sentinel, revokes, and requires all eight to read back 0: `[TEST] name=device_revocation_quiesce result=PASS transports=8 stride=0x200 next_owner_sees_reset=1 neighbours_untouched=1 rejected=6`. The write loop is driven against a poisoned scratch RAM page, because pointing it at the live PL011 or the virtio transport the block driver is using — to prove an address calculation — would mean breaking the console or the device under test; RAM is identity-mapped in the kernel, so the same code path executes and the result can be read back exactly. Known limitation: a scratch page cannot model the device's own response to the reset. That all eight `Status` registers receive 0 is proven here; that a real virtio device thereupon abandons its virtqueue is the spec's contract, not something this test observes.
...
- [x] **OBS-003** Contended records are deferred into lock-free rings and are now formatted asynchronously off the timer tick. The deferral had only ever been half-built: `printk()` gives up after 4096 lock attempts, appends a bare `printk_contention` record and drops the message — and nothing anywhere read those records back, so a dropped console line was recoverable only by attaching a debugger and walking `emergency::buffers` by hand, and not for long, because the ring wraps. Two things had to be got right, and the first attempt at each was wrong in a way only measurement exposed. (1) **Draining the per-CPU trace rings cannot work.** Under `CONFIG_TRACE` those rings carry every IRQ, IPC and context switch; measured, they wrap several hundred times a second, so a contention record is always overwritten before a once-per-tick drain limited by console bandwidth can reach it. The first implementation drained them directly and emitted 550 lines of nothing but its own loss counter, and the resulting console flood failed ACCEPTANCE outright. Reportable kinds — contention, fatal exception, stack corruption, user fault, device assign/revoke — are now mirrored at `append()` time into a dedicated 64-entry ring whose occupancy tracks diagnostics rather than trace volume; the certification test fails if a trace kind ever leaks into it, because that would silently regress the fix to the broken version. (2) **The drain must never spin on the console lock.** It runs off the timer interrupt, so it can interrupt the lock holder *on its own CPU*, where no amount of spinning can win — the holder cannot run to release it until the handler returns. With an unconditional `printk()` the drain was called and produced no output at all; it now reads the lock word first and skips the tick if it is held, and if another CPU takes it in between, `printk` returns −1 and the cursor stays put so the record is retried rather than dropped. Verified by real records on a live boot rather than synthetic ones: 11–15 lines per run covering five of the six reportable kinds, including two genuine `printk-contention` records from CPUs 1 and 3 — console lines that really were dropped and were previously invisible — and a `user-fault` carrying syndrome `0x92000047` with its fault address and instruction pointer. Interleaved same-session A/B over three pairs showed no latency cost (change 236k/429k/517k ticks against HEAD 420k/420k/571k). Known limitation: the drain is bounded to four records per tick, so a sustained diagnostic storm is reported as a `lost=` count rather than in full.
...
- [x] **OBS-008** A release-enabled, sequence-published bounded ring audits VM reset, mapping, run, pause/resume, stop and teardown, and device assignment is now recorded too. Device records go to the kernel-core emergency ring rather than the hypervisor audit ring, deliberately: that ring is VM-scoped, and a device here is assigned to a driver **process** — root mints its MMIO frame and interrupt line — so filing one against a vmid would mean inventing one. Emitted with `append()` rather than `trace()` so they survive a release build, from `interrupt::bind` and `release_ownership` (the line changing hands, and authority being withdrawn by revoke or teardown) and from `create_device_frame` and `release_frame_mappings` (the MMIO window). Verified rather than assumed, since the ring is write-only from the system's point of view: `device_assignment_audit` scans it and reports `assigns=3 revokes=1 malformed=0`, also rejecting any record that names no device.
...
- [x] **TST-005** VM lifecycle unit tests implemented. `tests/.../hypervisor/vm_lifecycle.hh` drives the refusal paths the integration suites never reach, since a healthy sequence does not: statically allocated VM/vCPU objects refuse destruction outright; each child-accounting guard (`active_vcpus`, `vcpu_count`, `mapping_count`) refuses independently, so none is load-bearing only in combination; `pause`/`stop` refuse while a vCPU is executing; the pause/resume state machine rejects illegal transitions in both directions; and `stop` disarms a pending virtual interrupt rather than leaving it to deliver into a context that is no longer running.
- [x] **TST-006** Stage-2 table unit tests implemented. `tests/.../hypervisor/stage2_tables.hh` covers the positive path the existing suite had none of: `stage2_leaf_descriptor` bit-for-bit against architectural meaning (output frame preserved, S2AP widened by write and by nothing else, XN cleared by execute and by nothing else, device memory carrying neither the normal-memory attributes nor inner-shareable), and `stage2_tracking` query/clear semantics (out-of-range is `not_found` rather than zeroed success, a read does not mutate, a clear reports the pre-clear state, and the generation skips zero on wrap since zero means never queried). The rejection paths — overlap, W^X, alignment, missing unmap — were already covered by the certification expected-error suite.
...
- [x] **TST-013** Real multi-vCPU guest integration test exists and passes: `hypervisor_real_smp_execution`, reported as `[HV-REAL-SMP] physical-cpus=4 guest-streams=4 barrier=4 irqs=4 migrations=4 result=PASS`. Genuine execution, not modelled — `tests/.../hypervisor/control_models.hh` prepares a bootstrap guest, configures each vCPU with a real reset PSTATE and sanitized `SCTLR_EL1`, enters them with `run()` on four physical CPUs, and reports guest PC on exit, across entry, migrate and interrupt-reentry phases. Distinct from `[HV-MODEL-0.3] multi-vcpu-multivm-lifecycle`, which is explicitly `execution=modeled`.
- [x] **TST-014** Concurrent two-VM execution test exists and passes: `hypervisor_real_multivm_isolation`, alongside `[HV-S] concurrent-multivm vms=2 vcpus=6 result=PASS`, `[HV-0.4] concurrent-vcpu-multivm-execution result=PASS`, and `[HV-AD] concurrent-multivm-reentry switches=192 result=PASS`. Two VMs with six vCPUs executing concurrently with isolation verified, again real rather than modelled.
...
- [x] **TST-018** `ipc_lifecycle_races` covers explicit cancel, timer expiry, blocked destroy, server exit with live reply authority, and endpoint reuse across CPUs, and the instruction-level race fuzz that was missing now exists. The scripted test reaches each interesting state deliberately, which proves the transitions are right and says nothing about the interleavings -- a scripted test chooses when every step happens, and these bugs are the ones that only occur when nobody chose. `tests/.../ipc/lifecycle_race.hh` puts three CPUs on the endpoint queue through the reschedule-IPI work lane while CPU 0 drives the retire/re-initialise cycle against them, checking `validate()` after every single operation. Two properties justify the harness: the queue arithmetic (`sender_head`, `sender_tail` and `sender_count` must agree through wraparound, and only do so for as long as the endpoint lock really serialises them -- this kernel has already shipped one lock whose fairness bug appeared only under real contention, so "the lock works" is not free), and reuse (retiring is safe only if no sender slips in after `retiring` is observed). Result: `[TEST] name=ipc_lifecycle_race_fuzz result=PASS cpus=3 enqueues=106 dequeues=103 reuse_cycles=454 deepest=3 wraparounds=4 anomalies=0 queue_consistent=1 retire_respected=1`. `deepest=3` means all three CPUs held a queued sender at the same moment, and the figures move run to run (deepest 1-3, wraparounds 3-7, reuse cycles 14-520), which is the interleaving genuinely varying rather than a fixed schedule. Final accounting is done by counting occupied slots by hand and comparing against `sender_count`, because the two drifting apart is exactly what a lost wraparound produces and is invisible to any check that reads only one of them. Two test-side faults had to be fixed first, both of which are worth recording because each initially looked like a kernel bug: workers re-enqueueing their own reference tripped `validate()`s duplicate rejection 230 times (a thread already blocked on send is never queued twice, so the workers now honour that precondition), and using `user_threads[1..3]` failed `validate()` exactly once per enqueue because `CONFIG_ROOT_ONLY_BOOT` registers only thread 0 -- the workers now register their own bare thread-typed headers and hand them back afterwards. Interleaved same-session A/B showed no latency cost (change PASS 564k/384k against HEAD 679k FAIL / 432k PASS). Fault injection is now covered for the property that most needed it: `tests/.../ipc/transfer_rollback.hh` fails the second mint of a three-capability transfer and asserts the batch rolls back completely, then that the same batch succeeds once nothing is armed. CSPACE_DESIGN states transfer is all-or-nothing, and the duplicate and occupied-destination cases were already reachable by argument, but a mint FAILURE partway through needs the derivation table exhausted at exactly the right moment — so the rollback loop written for it had never executed. A receiver holding two of three capabilities plus an error would be in a state no caller is written to handle.
- [x] **TST-019** Deterministic capability-revoke-driven unmapping integration test exists, and the concurrent revoke/map/unmap and TLB-shootdown races are now fuzzed under real parallelism. The deterministic test proves the single-CPU ordering and can say nothing about the case the mapping lock exists for: one CPU tearing every mapping off a frame while others add and remove their own. That window is narrow, reachable only with genuine parallelism, and losing it does not fail cleanly — a mapping record leaked or double-freed leaves `mapping_count` disagreeing with `mappings[]`, a corruption the system carries silently until something much later trips over it. Three CPUs now map and unmap the same frame at their own addresses while CPU 0 repeatedly calls `unmap_all()` on it. That is the operation capability revocation actually drives, so this is the revoke path rather than a stand-in, and since every unmap issues a TLB shootdown the shootdown races come along without a separate harness. Dispatch reuses the reschedule-IPI work lane the hypervisor SMP test established; the completion wait is bounded so a CPU that never reports fails the test instead of hanging the boot. The assertion is not merely "no call returned an unexpected error" — after the race it counts the valid mapping records by hand and requires agreement with `mapping_count`, requires `mapping_database_valid()`, and requires the frame to give back exactly the page it took. Result: `[TEST] name=concurrent_revoke_map_unmap result=PASS cpus=3 operations=768 teardowns=5239 anomalies=0 mapping_count=0 database=valid pages_balanced=1`; teardown counts ranged 545–5239 across runs, which is the interleaving genuinely varying rather than a fixed schedule. Two things had to be measured rather than assumed. The first base address, 0x30000000, failed *every* map with `invalid_argument`: the arch layer will allocate a missing L3 table for a block but not the L2 above it, so a VA in a region the space has no coverage for is simply unmappable — the race now runs at 0x20100000, inside a block root already has tables for, which also keeps it measuring the mapping database rather than page-table allocation. The second: at 512 iterations per CPU the certification latency bounds failed, because the job body runs in interrupt context and those bounds track a *running* maximum, so a long loop here raises a number checked seconds later in an unrelated test. At 128 iterations an interleaved same-session A/B showed no cost (change 505k/409k ticks against HEAD 461k/593k) and more iterations buy repetition rather than coverage.
- [x] **TST-020** `tests/.../scheduling/configure_fuzz.hh` fuzzes the operation a migration goes through: 4096 generated configurations (848 accepted, 3248 rejected in the recorded run) assert that acceptance matches the contract derived independently, that a rejection moves not one field, and that an acceptance leaves no stale consumed budget, donated ticks, donation depth or replenishment behind. That is the all-or-nothing property `scheduling_configure`'s suspension requirement depends on — control.hh's stated reason for it is that a remote scheduler tick must never observe a partially rewritten context. Racing reconfiguration against a live thread across CPUs is now covered too, by the kernel-side driver rather than a privileged fuzz worker — the existing four-CPU root fuzz still cannot host it, because its workers are deliberately non-root and hold no thread-control capability to migrate with. `tests/.../scheduling/configure_race.hh` has CPU 0 attempt reconfiguration in a loop through the same guard the syscall applies, while the other three CPUs flip the thread between suspended and running underneath it. Two properties: an attempt that lands while the thread is running must be refused with `busy` and leave the context untouched, and the observers — reading the whole context back — must always see wholly one configuration or wholly the other, never a mixture. The two profiles differ in *every* field so a mixture cannot be mistaken for either. Result: `[TEST] name=scheduling_configure_race result=PASS cpus=3 accepted=41009 refused=23644 observations=384 torn=0 applied_while_running=0 flips=384`. Both outcomes are *required*, not merely reported: a run where every attempt happened to find the thread suspended would prove nothing about the guard, so the test fails if `accepted` or `refused` is zero — 23,644 attempts really were turned away mid-race. The observers take the lifecycle lock, modelling a remote tick that synchronises rather than one that does not; an unsynchronised reader would report tearing no real caller can see, since a suspended thread is on nobody's run queue, and that is the point of the suspension requirement rather than a gap in it. The victim is a standalone thread object on no run queue, so flipping its state cannot disturb anything actually scheduled.
...
- [x] **TST-023** Certification exercises allocation/accounting, nested extent split/retype/reclaim, twenty-way fragmentation/coalescing, metadata reuse, 32 repeated quota-exhaustion/reclaim cycles covering 512 frame lifecycles, balanced release, multi-map cleanup, attributes, and MMIO lifecycle; allocator exhaustion is now covered by `tests/.../memory/allocator_exhaustion.hh`: a bounded 512-page real burst asserting every allocation is aligned, distinct and accounted for and that release balances exactly, plus the at-empty contract (fails closed with `no_memory` specifically, since callers branch on it, returns no address, and the allocator is usable again afterwards). The empty state is reached by snapshotting and marking the bitmap full rather than by allocating 64k pages, because `allocate_physical_page` rescans every region from page zero on each call -- taking n pages costs O(n^2), which is a real property of the allocator recorded in 0167 rather than a limitation of the test. Multi-CPU pressure is now covered by `tests/.../memory/allocator_race.hh`, which drives all four CPUs against the allocator at once through the reschedule-IPI work lane. Every worker releases its page immediately, so the bitmap stays nearly empty and the rescan-from-zero behaviour keeps handing the same few low addresses to all four at the same moment — the worst case for mutual exclusion, chosen deliberately rather than arrived at by accident. Detection is direct rather than statistical: a worker stamps its page with a value unique to it, waits, and reads it back, so an allocator that hands the same page to two CPUs fails immediately. A test that merely compared returned addresses afterwards could miss exactly that — both CPUs release, the counts balance, and nothing looks wrong. Result: `[TEST] name=allocator_multicpu_race result=PASS cpus=4 allocations=512 collisions=0 misaligned=0 exhaustions=0 balanced=1`. Note on the gates: one smoke run during this work reported `FAIL (4 problem(s))` at host load ~9, and re-runs of both the change and HEAD then passed — it was the known ungated shell wedge (0156), not a regression, and is recorded here rather than omitted because the first instinct was to blame the change.
- [x] **TST-024** Attribute/mapping rejection and injected extent-split metadata failure verify transactional rollback with before/after invariant signatures; Systematic injection now exists: `verification::injection_site` names object registration, capability installation and page allocation, armed one failure at a time by `configure_failure()`, compiled to a constant `false` in release (the production header) and counting in certification (the shadowing test header). `tests/.../memory/fault_injection.hh` drives each site through `notification::create()` — the smallest operation that both registers a dynamic object and installs a capability naming it — and checks a before/after signature of the object table's live count, the target CSpace slot, and the free-page count, because an operation that fails loudly while leaking a registered object still "fails" and would satisfy a weaker check. Page allocation is armed deliberately as a negative control: notification creation allocates no page, so it must still succeed, proving the injector is targeted rather than global. Teardown injection now exists too, and it found a real defect on its first run. The `object_unregistration` site arms the point in `object::unregister_object()` just before the table exchange, modelling the failure that path genuinely returns — `busy`, when a concurrent destroyer wins the exchange. **The bug:** `notification::destroy()` revoked the capability *before* unregistering, so a failed unregister returned with the capability already gone and the object still registered and `allocated`. Nothing could name it to try again: a dynamic pool slot lost for the rest of the boot, and completely silent. `ipc::destroy()` had the same ordering plus one more hazard — it latches `retiring` first, so a failed teardown left an endpoint that could not be named *and* would have answered `busy` forever to any retry that could somehow reach it. Both are fixed by unregistering first and revoking afterwards, with the endpoint additionally rolling back `retiring` on failure. Reversing the order is safe for two reasons that had to be checked rather than assumed: the object table is generation-checked, so a capability naming a departed object fails closed on use and the window before the revoke grants no authority; and `revoke_reference_locked()` only scans cspaces comparing references, so it never resolves the object and does not care that it is already gone. Verified by `[TEST] name=fault_injection_rollback result=PASS sites=2 targeted=1 no_object_leak=1 no_capability_leak=1 no_page_leak=1 recovers=1 teardown_injected=2 teardown_retry_converges=2`, which requires the failed teardown to leave the live count *up by one* — the object survived intact rather than being half-removed — and then requires the unarmed retry to converge completely. The endpoint case is tested separately rather than assumed to follow from the notification one, because the `retiring` rollback is unique to it. **Known residual:** `memory::destroy_frame()` has the same revoke-then-unregister order and was deliberately left alone. Swapping it there would not make the retry converge, because `release_frame()` has already run by that point and is not idempotent, so the retry fails `not_found` either way. The leak is real, but the fix is a restructuring of that function rather than a reordering, and a half-change would alter a core path without making it correct.
...
- [-] **TST-030** Partially evidenced, and the distinction matters. What exists: `rt_logical_time_soak` reports `hours=6 periods=21600 violations=0`, but that is a LOGICAL-time soak — it advances a simulated clock through 21,600 period boundaries checking sporadic-server budget, throttling and eligibility accounting, not real threads meeting real deadlines. Alongside it, genuinely measured against real ticks: `scheduler_latency_bounds` and `ipc_latency_bound` against a 620000-tick limit, plus `preemption_latency`, `cross_cpu_wake_latency` and `interrupt_latency` metrics, and `sporadic_server_replenishment`, `priority_inheritance` (`inherited=240 base=20`) and `donation_chain_bound` (`maximum=8`). Open: a defined RT workload of real periodic threads with stated deadlines, run long enough for a miss to be meaningful. Recording the logical soak as if it were that would repeat exactly the model-versus-real confusion DOC-005 indexes the retired profiles to prevent.
...
- [x] **DOC-001** Implemented architecture documented in `docs/architecture/ARCHITECTURE.md` (the requirement's historical `arch_design.md` filename; placed under `docs/architecture/` per the layout `docs/README.md` defines and `tools/doc/check_layout.sh` enforces). Covers privilege layout, the header-only implementation form, the capability authority model including device authority, the compile-time bounds table and the hard 4-CPU requirement, lock and scheduling discipline, memory and ASID handling, the userspace service graph, the hypervisor, and build profiles — each pointing at the maintained document that owns the detail, and stating explicitly that the readiness checklist is authoritative on disagreement.
- [x] **DOC-002** Detail design is generated rather than hand-maintained, so it cannot drift: `tools/doc/collect.py` aggregates the module document beside each source module into `zilch_design.md` (359 modules, ~4500 lines), and refuses to build while any module lacks one. That check had 24 userspace modules missing — libc, the ext2 reader and on-disk format, the VFS server and its ramfs, the coreutils role table, the shell, the coreutils themselves and the boot probes — which is now zero, so `make doc-check` passes end to end for the first time.
- [x] **DOC-003** Every mandatory requirement has a stable ID in this checklist.
- [x] **DOC-004** Every requirement marked complete maps to implementation and evidence, enforced rather than asserted. `tools/doc/check_evidence_matrix.sh` (in `make doc-check`) expands the matrix's range and list notations, ignores the historical batch tables the matrix itself declares superseded, and separates COMPLETE-but-unmapped from open-but-unmapped. It found 32 completed requirements with no evidence row when written; that count is now **0**, held by a ratchet that fails if it ever rises. Open requirements are deliberately not required to carry rows -- they have no implementation to map -- so what the gate catches is the case this item exists to prevent: a requirement newly marked complete without evidence.
- [x] **DOC-005** Model-only runtime results use `HV-MODEL` and `hypervisor_control_model`. The legacy profile documents were already archived under `docs/history/profiles/`; they are now indexed by a README there rather than retrofitted with the newer naming, because editing historical records to agree with the present destroys their value as evidence. That index states what a reader needs: none of them is a current claim, each profile's bounds are literal (0.1 is one VM, one vCPU, 64 KiB guest RAM), `-model`-suffixed results exercised the control plane rather than guest instructions, and the checklist is authoritative for present state.
...
- [x] **DOC-009** Capability and IPC semantics documented formally enough for independent implementation, across five documents: `CAPABILITY_IPC_SEMANTICS.md` (rights and their attenuation rules, generation checking, the ARM64 syscall register convention that `ABI_COMPATIBILITY.md` freezes but never stated, the five IPC operations, thread states, receive ordering and its lock discipline, one-shot reply capabilities including the overwrite hazard and scheduling donation, all-or-nothing capability transfer, timeout encoding, notification semantics, and the full error taxonomy), `CSPACE_DESIGN.md` (selector geometry, capacity, locking), `IPC_BADGE_PROTOCOL.md` (badge capture and identity), `FAULT_IPC_PROTOCOL.md` (fault delivery), `LOCKING_PROTOCOL.md` (lock rank and acquisition discipline).
...
- [x] **DOC-012** Hypervisor guest-visible architecture documented in `docs/readiness/HYPERVISOR_GUEST_ARCHITECTURE.md`: execution model and stage-2 contract, the exit taxonomy and what each reports, system-register handling, and what a guest actually sees — manifest-declared memory, boot state, the trapped-and-emulated vPL011 (and why passthrough of the real UART would collide with console-server ownership), the vCPU-resident virtual GIC CPU interface, and the virtual timer. It also states plainly what is absent (guest-facing PSCI, distributor/redistributor emulation, a constructed device tree, storage, virtio), which is what bounds USR-030/031.
