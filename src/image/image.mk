...
$(EARLYFS): $(USER_PROGRAM_ELFS) $(if $(filter 1,$(CONFIG_HYPERVISOR_SELFTEST)),$(GUEST_TEST_ELF),) $(MANIFEST)
	@mkdir -p $(dir $@)
	@printf '  EARLYFS %s\n' '$@'
	@$(SRCTREE)/tools/image/make_earlyfs.sh $(USER_OBJDIR)/init.elf $(USER_OBJDIR)/memory-server.elf $(USER_OBJDIR)/pager-client.elf $(USER_OBJDIR)/memory-client.elf $(USER_OBJDIR)/domain-manager.elf $(MANIFEST) $@ $(if $(filter 1,$(CONFIG_HYPERVISOR_SELFTEST)),$(GUEST_TEST_ELF),-)
