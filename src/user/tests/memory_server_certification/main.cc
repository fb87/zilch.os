...
namespace
{
     ...
     * faults -- exercised only by this pager certification sequence (real
     * demand paging, USR-010, isn't implemented yet). The production
    ...
    [[noreturn]] void fail(sys::word_t code) noexcept {
        (void)sys::control(sys::abi::v1::control_operation::notification_signal,
                           sys::memory_service::notification, failure_badge_base | code);
        for (;;)
            asm volatile("" ::: "memory");
    }
} // namespace
...
extern "C" int main(sys::word_t, sys::word_t) noexcept {
    for (sys::word_t index = 0U; index < pager_client_count; ++index) {
        const sys::word_t created =
            sys::control(sys::abi::v1::control_operation::resource_frame_create,
                         sys::memory_service::resource, pager_frame);
        ...
        const sys::word_t signalled =
            sys::control(sys::abi::v1::control_operation::notification_signal,
                         sys::memory_service::notification, completion.message2);
    ...
    if (sys::control(sys::abi::v1::control_operation::notification_signal,
                     sys::memory_service::notification,
    ...
    sys::thread_exit(0U, sys::memory_service::notification, server_completion_badge);
}
