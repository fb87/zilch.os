...
namespace sys::kernel::memory
{
    ...
    inline error_t initialize_objects() noexcept {
        ...
        for (u32 index = 0U; index < page_table_count; ++index) {
            ...
            const error_t result = object::register_object(page_tables[index].object,
    ...
    [[nodiscard]] inline error_t map(space::address_space& target, frame& source, vaddr_t address,
        ...
        if (!valid_wx(permissions) || source.mapped)
            return error_t::denied;
        const error_t result =
            target.map_page(address, source.bytes, writable(permissions), executable(permissions));
        if (result != error_t::success)
            return result;
        source.mapped = true;
    ...
    [[nodiscard]] inline error_t unmap(space::address_space& target, frame& source) noexcept {
