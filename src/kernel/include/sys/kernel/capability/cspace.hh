...
namespace sys::kernel::capability
{
    ...
    [[nodiscard]] inline error_t install(cspace_t& cspace,
                                         capability_id_t selector,
                                         const object::reference_t& object,
                                         rights_t rights) noexcept
    {
        if (selector >= cspace_slot_count
            || object.type == object::type_t::none
            || object::resolve(object) == nullptr) {
            return error_t::invalid_argument;
        }
        slot_t& slot = cspace.slots[selector];
        if (slot.object.type != object::type_t::none) return error_t::busy;
        slot.object = object;
        slot.rights = rights;
        return error_t::success;
    }

    [[nodiscard]] inline error_t remove(cspace_t& cspace,
                                        capability_id_t selector) noexcept
    {
        if (selector >= cspace_slot_count) return error_t::invalid_argument;
        slot_t& slot = cspace.slots[selector];
        if (slot.object.type == object::type_t::none) return error_t::not_found;
        slot = {};
        return error_t::success;
    }

    [[nodiscard]] inline error_t lookup(const cspace_t& cspace,
                                        capability_id_t selector,
                                        object::type_t expected_type,
                                        right_t required,
                                        object::header_t*& result) noexcept
    {
        result = nullptr;
        if (selector >= cspace_slot_count) return error_t::not_found;
        const slot_t& slot = cspace.slots[selector];
        if (slot.object.type != expected_type) return error_t::denied;
        const error_t rights_result = validate(slot, required);
        if (rights_result != error_t::success) return rights_result;
        result = object::resolve(slot.object);
        return result != nullptr ? error_t::success : error_t::not_found;
    }

    [[nodiscard]] inline error_t copy(cspace_t& destination,
                                      capability_id_t destination_selector,
                                      const cspace_t& source,
                                      capability_id_t source_selector,
                                      rights_t rights_mask) noexcept
    {
        if (destination_selector >= cspace_slot_count
            || source_selector >= cspace_slot_count) return error_t::invalid_argument;
        const slot_t& source_slot = source.slots[source_selector];
        if (source_slot.object.type == object::type_t::none
            || object::resolve(source_slot.object) == nullptr) return error_t::not_found;
        if (!source_slot.rights.contains(right_t::grant)) return error_t::denied;
        const u32 granted = source_slot.rights.bits & rights_mask.bits;
        if (granted == 0U) return error_t::denied;
        lock(destination);
    ...
    [[nodiscard]] inline error_t move(cspace_t& destination,
                                      capability_id_t destination_selector,
                                      cspace_t& source,
                                      capability_id_t source_selector) noexcept
    {
        ...
        lock(*first);
        if (second != first) lock(*second);
        slot_t& source_slot = source.slots[source_selector];
        slot_t& destination_slot = destination.slots[destination_selector];
        error_t result = error_t::success;
        if (source_slot.object.type == object::type_t::none) result = error_t::not_found;
        else if (destination_slot.object.type != object::type_t::none) result = error_t::busy;
        else {
            destination_slot = source_slot;
            source_slot = {};
        }
        if (second != first) unlock(*second);
        unlock(*first);
        return result;
    }
    ...
    inline u32 revoke_in(cspace_t& cspace,
                         const object::reference_t& reference) noexcept
    {
        ...
        for (u32 index = 0U; index < cspace_slot_count; ++index) {
            ...
            if (slot.object.id == reference.id
                && slot.object.generation == reference.generation
