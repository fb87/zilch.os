...
namespace sys::kernel::acceptance
{
    ...
    inline void mark_profile_self_tests(bool authority, bool memory,
    ...
    {
        const bool previous = __atomic_exchange_n(
            &fault_ipc_observed, true, __ATOMIC_ACQ_REL);
        if (!previous) report("fault_ipc_delivery", true);
    }
    ...
    inline void report_final(u64 operations, u64 failures,
                             bool all_cpus_progressed) noexcept
    {
        if (operations < certification_operation_target || failures != 0U
            || !all_cpus_progressed || !prerequisites_ready()) return;
        bool expected = false;
        if (!__atomic_compare_exchange_n(&final_reported, &expected, true,
